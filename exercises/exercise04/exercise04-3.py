...
def plot_losses(loss_histories):
    for learning_rate in LEARNING_RATES:
        ...
        plt.plot(np.arange(NUM_EPOCHS), loss_histories[learning_rate], label=label)

    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.yscale("log")
    plt.title("Effect of Varying Learning Rates on Training Loss")
    plt.legend()
    plt.savefig("lr_losses.png")
