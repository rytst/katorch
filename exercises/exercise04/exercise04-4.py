...
def plot_losses(loss_histories):
    for momentum in MOMENTUMS:
        label = "No Momentum" if momentum == 0 else f"mom = {momentum}"
        plt.plot(np.arange(NUM_EPOCHS), loss_histories[momentum], label=label)
    ...
    plt.legend()
    plt.savefig("momentum_losses.png")
...
def main():
    ...
    for momentum in MOMENTUMS:
        ...
        loss_histories[momentum] = loss_history
